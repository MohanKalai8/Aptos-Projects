...
module my_addrx::Staking {
    ...
    public fun stake(acc_own: &signer, amount: u64) {
        ...
        assert!(balance >= amount, EINSUFFICIENT_BALANCE);
        assert!(!exists<StakedBalance>(from), EINSUFFICIENT_BALANCE);
        my_addrx::BasicTokens::withdraw(from, amount);
        let staked_balance = StakedBalance{staked_balnce: amount};
        move_to(acc_own, staked_balance);
    }

    public fun unstake(acc_owner: &signer, amount: u64) acquires StakedBalance {
        let from = signer::address_of(acc_owner);
        let staked_balance = borrow_global_mut<StakedBalance>(from);
        let staked_amount = staked_balance.staked_balnce;
        ...
        staked_balance.staked_balnce = staked_amount - amount;
    }

    public fun claim_reward(acc_own:&signer) acquires StakedBalance {
        let from = signer::address_of(acc_own);
        let staked_balance = borrow_global_mut<StakedBalance>(from);
        let staked_amount = staked_balance.staked_balnce;   
    ...
    public entry fun test_staking(alice : signer, bob : signer) acquires StakedBalance {
        ...
        // check that Alice's staked amount is correct
        let alice_resource = borrow_global<StakedBalance>(signer::address_of(&alice));
        assert!(alice_resource.staked_balnce == 500, 100);

        // Alice unstakes some tokens
        unstake(&alice, 200);
        let alice_resource = borrow_global<StakedBalance>(signer::address_of(&alice));
        assert!(alice_resource.staked_balnce == 300, 100);
